...
import { NFT721_ADDRESS } from '@/lib/contracts/addresses';
import { SharedNFT721Abi } from '@/lib/contracts/abis';
...
export function useMintNFT() {
  ...
  const mint = async (data: MintData): Promise<MintResult> => {
    ...
    try {
      ...
      const { request } = await publicClient.simulateContract({
        ...
        abi: SharedNFT721Abi,
        functionName: 'mint',
        args: [BigInt(data.onChainId), tokenURI],
        account: address,
      });
      ...
      // Prefer S3 URL for fast image loading, fallback to IPFS gateway
      ...
      const mintResult: MintResult = {
        ...
        nftId: registeredNft.id, // Backend UUID
      };
